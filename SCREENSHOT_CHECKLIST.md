...
| 2026-08-31 | Tally Sheets — AI Import dialog + grid review | `/screenshots/tally/ai-import-dialog__01.png` (+ `ai-import-review__01`) | Now stable at tms-ng v5.45.54. Two shots: (1) the AI Import dialog preview showing the inline "− N +" Add-to-Length stepper with live example + "save as default" checkbox + amber uncertain cells + the "N rows need a value" note; (2) the ROUND grid after import showing the amber review bar ("N rows need a look" + Show-only-these), an amber-marked cell, and the floating row-crop panel (bottom-right). Reach it: Round tally sheet → toolbar → AI Import; upload a real handwritten sheet with 2-digit lengths; import; then click a flagged cell to trigger the crop panel. ROUND, no admin-only state. |
