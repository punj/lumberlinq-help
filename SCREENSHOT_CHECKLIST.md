...
|------|--------------------|------------------|--------|
| 2026-08-24 | Dashboard, Utility, Storage & Support — Unified Search (Records/Stock modes) | `/screenshots/platform-basics/platform-basics__unified-search-records__01.png` (+ `__stock__01`) | Auto and Calculator modes are captured, live, and now in the article (see Done) — Records and Stock modes still need a capture, plus real search text typed in to show results, not an empty box. Updated 2026-08-25: Records mode now also shows a "Pages" result group (matching sidebar menu items) and an "Ask Linc" fallback card below the results — make sure the capture's search text is chosen so both new elements are visible, not just record matches |
