...
| 2026-08-31 | Tally Sheets — AI Import dialog | `/screenshots/tally/ai-import-dialog__01.png` | The AI Import dialog is gaining new elements across tms-ng Phases 1–3: an "Add to Length" status line + a preview-step "Apply +N / Keep as written" prompt (v5.45.51, live), and later a keep-every-row `null`-cell treatment and per-cell uncertainty markers + zoomed row crop. Capture ONCE after Phase 3 ships, not per phase. Reach it: a Round tally sheet → toolbar → AI Import; for the "Apply +N" prompt, first set an "Add to Length" value in the sheet Settings then toggle it off, and upload a sheet with 2-digit lengths. Logged as ROUND — no admin-only state involved. |
