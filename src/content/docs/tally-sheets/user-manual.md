...
- After the rows are in the sheet, the Round grid keeps a **"Pages" bar** above it (edit view only — it never appears on a share link): one chip per page, click a chip to jump to that page's first row, and the chip highlights as you scroll. The first row of each page also has a coloured divider line with a **"Page N"** marker, and this stays after you save and reload. Switch the marker between a divider label (default) and a small chip in the SR column under **Settings → Configuration Settings → Columns → "AI Import page marker"**. Rows you type by hand, and sheets imported before this feature, have no page and show no marker.
